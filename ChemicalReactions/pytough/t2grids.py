...
class t2block(object):
    ...
    def get_num_connections(self):
        return len(self.connection_name)
...
class t2grid(object):
    ...
    def get_block_centres_defined(self):
        if self.num_atmosphere_blocks == 1:
            istart = 1
        else:
            istart = 0
    ...
    def calculate_connection_centres(self, geo, blockmap={}):
        ...
        def imapblock(blk):
            return iblockmap[blk] if blk in iblockmap else blk
        ...
        for con in self.connectionlist:
            ...
            if layernames[0] == layernames[1]:  # horizontal connection
                ...
                for colname, blk in zip(colnames, con.block):
                    ...
                    if blk.centre is not None:
                        vcentre = blk.centre[2]
                    else:
                        vcentre = geo.block_centre(lay, col)[2]
    ...
    def embed(self, subgrid, connection):
        ...
        if subvol < hostblock.volume:
            ...
            if len(dupblks) == 0:
                ...
                print('Grid embedding error: the following blocks are in both grids:', dupblks)
        else:
            print('Grid embedding error: the host block is not big enough to contain the subgrid.')
        return result
    ...
    def add_rocktype(self, newrocktype=None):
        ...
        if newrocktype.name in self.rocktype:
            ...
            self.rocktypelist.append(newrocktype)
    ...
    def write_vtk(self, geo, filename, wells=False, blockmap={}, surface_snap=0.1):
        ...
        if hasattr(writer, 'SetInput'):
            writer.SetInput(vtu)
        elif hasattr(writer, 'SetInputData'):
        ...
        for ilay, lay in enumerate(geo.layerlist[1:]):
            ...
            for icol, col in enumerate(geo.columnlist):  # vertical connections
                ...
                if ilay == 0:  # atmosphere connections
                    ...
                    if atmos_type == 0:
                        aboveblk = grid.blocklist[0]
                    elif atmos_type == 1:
                        aboveblk = grid.blocklist[icol]
                    else:
                        continue
                else:
    ...
    def rectgeo(self, origin_block=None, atmos_volume=1.e25, remove_inactive=False,
        ...
        def block_spacings(grid, ob, max_volume):
            ...
            for dirn in range(1, 4):
                if dirn < 3:
                    ...
                    blk = topmost_block(grid, max_volume)
                ...
                if colblocks:
                    ...
                    if topblock.volume > 0.:
                        ...
                        if layerthicks:
                            ...
                            lt = block_height
                        if block_height <= lt:
                            surface = zc + 0.5 * block_height
                        else:
                            surface = zc - 0.5 * lt + block_height
        ...
        if not required_centres_present(self, atmos_volume):
            ...
            if isinstance(origin_block, str):
                ob = self.block[origin_block]
            elif isinstance(origin_block, t2block):
                ob = origin_block
            else:
                ob = find_origin_block(self)
            if ob is None:
                raise Exception("Can't find origin block for grid.")
            else:
