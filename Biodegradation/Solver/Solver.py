...
class Solver(object):
    ...
    def mult_five(self, y, t, params):
        ...
        c5 = Component(5, S5, 'VC', umax5, Ks5, Kci5, Khi5, Knci5)
        all_comp = [c1, c2, c3, c4, c5]
        # iscompetiting = [{c1: []}, {c2: [c1]}, {c3: [c2]}, {c4: [c2, c3]}]
        # ishaldane = [{c1: []}, {c2: [c2]}, {c3: [c3]}, {c4: [c4]}]
        soln = Solution(all_comp, self.iscompeting, self.ishaldane, self.isnoncompetiting)
        inhibit = Inhibition(soln)
        monod1 = Monod().growth_rate_new(self.components[0], inhibit)
        monod2 = Monod().growth_rate_new(self.components[1], inhibit)
        monod3 = Monod().growth_rate_new(self.components[2], inhibit)
        monod4 = Monod().growth_rate_new(self.components[3], inhibit)
        monod5 = Monod().growth_rate_new(self.components[4], inhibit)
        #
        # derivs = [-monod1 * X,
        #           -monod3 * X + monod1 * X,
        #           -monod2 * X + monod3 * X,
        ...
        for component in all_comp:
            deriv.append(self.computeDerivative(self.getReactants(component), self.getProducts(component), all_comp, X))
